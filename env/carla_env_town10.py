...
class InterSection(gym.Env):
    def __init__(self, enabled_obs_number=8, vehicle_type='single', use_checker=False,
        ...
        if loaded_map != 'Town10HD_Opt':
            raise RuntimeError(
                f"CARLA server loaded {loaded_map}, expected Town10HD_Opt. "
                "Start CARLA with /Game/Carla/Maps/Town10HD_Opt."
            )
    ...
    def reset(self):
        ...
        self.agent = BasicAgent(self.ego_vehicle, target_speed=self.target_speed,opt_dict={'ignore_vehicles': True})
        ...
        blueprints = [x for x in blueprints if (int(x.get_attribute(
            'number_of_wheels')) == 4 and x.id != 'vehicle.volkswagen.t2' and x.id != 'vehicle.bmw.isetta')
    ...
    def record_one_step_total(self):
        ...
        for obs_id in range(len(self.obs_actors)):
            self.vehicle_state_dict['v_' + str(obs_id)] = self.get_actor_state_total(self.obs_actors[obs_id], 1)
    ...
    def get_observation_scene(self):
        ...
        if len(self.ego_location_history) == 1:
            ...
            step_dist = np.sqrt(
                (self.ego_location_history[-2][0] - x_ego) ** 2 + (self.ego_location_history[-2][1] - y_ego) ** 2)
    ...
    def step(self, action):
        ...
        info = (
            self.finish, self.collision, self.off_route, self.max_time, self.count, self.count_speed / (self.count + 1), self.vehicle_state_dict
        )
        self.count += 1
